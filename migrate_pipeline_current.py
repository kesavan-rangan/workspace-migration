...
def build_export_pipeline(client_config, checkpoint_service, args) -> Pipeline:
    ...
    export_libraries = pipeline.add_task(LibraryExportTask(client_config, args, checkpoint_service, wmconstants.CLUSTERS in skip_tasks), [export_instance_pools])
    export_jobs = pipeline.add_task(JobsExportTask(client_config, args, checkpoint_service, wmconstants.JOBS in skip_tasks), [export_libraries])
    export_repos = pipeline.add_task(ReposExportTask(client_config, args, checkpoint_service, wmconstants.CLUSTERS in skip_tasks), [export_jobs])
...
def build_import_pipeline(client_config, checkpoint_service, args) -> Pipeline:
    ...
    import_libraries = pipeline.add_task(LibraryImportTask(client_config, args, checkpoint_service, wmconstants.CLUSTERS in skip_tasks), [import_instance_pools])
    import_jobs = pipeline.add_task(JobsImportTask(client_config, args, checkpoint_service, wmconstants.JOBS in skip_tasks), [import_libraries])
    import_repos = pipeline.add_task(ReposImportTask(client_config, args, checkpoint_service, wmconstants.CLUSTERS in skip_tasks), [import_jobs])
