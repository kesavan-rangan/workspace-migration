...
def build_pipeline(args) -> Pipeline:
    ...
    if not args.import_pipeline:
        client_config['export_dir'] = os.path.join(client_config['base_dir'], session) + '/'
    else:
        client_config['export_dir'] = client_config['base_dir'] + '/'
...
def build_export_pipeline(client_config, checkpoint_service, args) -> Pipeline:
    ...
    # export_workspace_acls = pipeline.add_task(WorkspaceACLExportTask(client_config, checkpoint_service, wmconstants.WORKSPACE_ACLS in skip_tasks), [])
    # export_notebooks = pipeline.add_task(NotebookExportTask(client_config, checkpoint_service, wmconstants.NOTEBOOKS in skip_tasks), [])
    ...
    # export_clusters = pipeline.add_task(ClustersExportTask(client_config, args, checkpoint_service, wmconstants.CLUSTERS in skip_tasks), [])
    ...
    # export_metastore = pipeline.add_task(MetastoreExportTask(client_config, checkpoint_service, args, wmconstants.METASTORE in skip_tasks), [])
    ...
    export_users = pipeline.add_task(UserExportTask(client_config, checkpoint_service, wmconstants.USERS in skip_tasks), [])
    export_clusters = pipeline.add_task(ClustersExportTask(client_config, args, checkpoint_service, wmconstants.CLUSTERS in skip_tasks), [export_users])
    export_libraries = pipeline.add_task(LibraryExportTask(client_config, args, checkpoint_service, wmconstants.CLUSTERS in skip_tasks), [export_clusters])
    ...
    # export_notebooks = pipeline.add_task(NotebookExportTask(client_config, checkpoint_service, wmconstants.NOTEBOOKS in skip_tasks), [export_workspace_acls])
    # export_secrets = pipeline.add_task(SecretExportTask(client_config, args, checkpoint_service, wmconstants.SECRETS in skip_tasks), [export_notebooks])
    ...
    _finish_export = pipeline.add_task(FinishExportTask(client_config),
                                      [ export_libraries ])

    print(f"Please find the output files generated in this path: {client_config['export_dir']}")
...
def build_import_pipeline(client_config, checkpoint_service, args) -> Pipeline:
    ...
    if args.include_user_import:
        import_users = [pipeline.add_task(UserImportTask(client_config, checkpoint_service, wmconstants.USERS in skip_tasks))]
        pass
    else:
        logging.info("Skipping user import as --include-user-import is not specified in the command")
        import_users = []

    import_clusters = pipeline.add_task(ClustersImportTask(client_config, args, checkpoint_service, wmconstants.CLUSTERS in skip_tasks), [])
    import_libraries = pipeline.add_task(LibraryImportTask(client_config, args, checkpoint_service, wmconstants.CLUSTERS in skip_tasks), [import_clusters])
    ...
    # import_secrets = pipeline.add_task(SecretImportTask(client_config, checkpoint_service, wmconstants.SECRETS in skip_tasks), [import_workspace_acls])
